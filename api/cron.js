...
console.log('Cron: Booting up BlueSky follow bot...')

export const handler = async function () {
    console.log("Cron: Initialized.")
    
    var now = new Date();
    console.log('Cron: running a task every 5 minutes');
    // Log in to Bluesky
    ...
    try{
        ...
        console.log(`Cron: Found ${actors.length} profiles.`)
        let i = 0;
        ...
        for (const actor of actors) {
            ...
            if (profile.data.viewer.following == null) {await agent.follow(profile.data.did);
                console.log(`Cron: Followed ${profile.data.did} profile.`);
                // increment i by 1
                i++;
                await sleep(2000);
            }
        ...
        console.log(`Cron: Total Followed ${i} profiles.`)
    } catch (e) {
        console.log(e)
    }
    console.log('Cron: Completed async responses. Goodbye.')
